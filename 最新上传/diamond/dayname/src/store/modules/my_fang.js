...
// 
  ...
  user(ctx,postdata){
    ...
    axios.post("/api/user.do",postdata)
    .then(response=>{
  ...
  myappointment(ctx){
    ...
    axios.get('/api/myappointment.do').then(function(response){
      console.log(response.data)
      // 日期过滤
  ...
  confirm(ctx,str){
    console.log(str);
    axios.get("/api/myappointment.do?myappointment_state="+str).then(function(response){
      // 日期过滤
      ctx.commit('formatUTC',response.data);
      // 数据修改
      ctx.commit('myappointment',response.data);
  ...
  // 我的订单
  myorder(ctx){
    axios.get("/api/myorder.do").then(function(response){
      // 日期过滤
      ctx.commit('formatUTC',response.data);
      // 数据修改
  ...
  myintegral(ctx,str){
    ...
    axios.get("/api/myintegral.do?myintegral_myclass="+str).then(function(response){
      // 日期过滤
      ctx.commit('formatUTC',response.data);
      // 数据修改
  ...
  // 获取总积分
  allintegral:(ctx)=>{
    axios.get("/api/allintegral.do").then(function(response){
  ...
  mycertificate(ctx,str){
    ...
    console.log(str);
    axios.get("/api/mycertificate.do?code_state="+str).then(function(response){
      // 日期过滤
      ctx.commit('formatUTC',response.data);
      // 数据修改
  ...
  // 我的礼品部分
  mygift(ctx){
    axios.get("/api/mygift.do").then(function(response){
      // 日期过滤
      ctx.commit('formatUTC',response.data);
      // 数据修改
...
const mutations = {
  formatUTC(state,data){
    for(let i=0;i<data.length;i++){
      let utc_datetime=data[i].user_date;
      ...
      data[i].user_date=beijing_datetime;
    }
  },
  // 数据添加
  // 用户数据
